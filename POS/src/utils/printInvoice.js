...
export function printInvoiceCustom(invoiceData) {
	...
	if (isDraft) {
		...
		const addressDisplay =
			...
			"";
		const customerPhone =
			invoiceData.phone || invoiceData.contact_phone || invoiceData.mobile_no || "";
		...
		const invoiceDate = invoiceData.posting_date
			? invoiceData.posting_date.length > 10
				? invoiceData.posting_date.substring(0, 10)
		...
		const draftHeading = [
			"DRAFT",
			customerName,
			`${pad(stamp.getDate())}-${pad(stamp.getMonth() + 1)}-${stamp.getFullYear()} ${pad(
				stamp.getHours()
		...
		brandKeys.forEach((brand, brandIndex) => {
			...
			itemRowsHtml += `<div class="brand">${brand}</div>`;
			for (const item of itemsInBrand) {
				...
				const nm = (item.item_name || "").slice(0, 17);

				itemRowsHtml += `
				<div class="itm">
				<table class="r1"><tr>
					<td class="a0"><span class="sn">(${serialCount})</span></td><td class="a1">${nm}</td><td class="a2">${
					item.item_code || ""
				}</td>
				</tr></table>
				<table class="r2"><tr>
					<td class="a0">${qtyDisplay}</td><td class="b2">${itemPrice}</td><td class="b3">${
					showRack ? item.custom_rack_identifier || "" : ""
				}</td><td class="b4"></td>
				</tr></table>
				</div>
				`;
...
<head>
    ...
    <style>
        ...
        .r1, .r2 {
            ...
            font-size: 9.5px; line-height: 1.2; margin: 0;
        }
        .r1 td, .r2 td { vertical-align: top; white-space: nowrap; overflow: hidden; }
        .r1 td.a0, .r2 td.a0 { width: 8%; }
        .r2 td.a0 { text-indent: 22px; overflow: visible; }
        .r1 td.a1 { width: 44%; text-align: center; }
        .r1 td.a2 { width: 48%; }
        
        .r2 td.b2 { width: 44%; text-align: center; }
        .r2 td.b3 { width: 26%; }
        .r2 td.b4 { width: 22%; }
        .hd td { font-size: 8.5px; }
        .sn { font-size: 7.5px; vertical-align: super; }
        .itm { margin-bottom: 5px; }
        .brand { font-size: 9px; margin: 3px 0 1px; font-weight: bold; }
    </style>
</head>
<body>
...
<table class="r1 hd"><tr>
    <td class="a0">S No.</td><td class="a1">Item Name</td><td class="a2">Item Code</td>
</tr></table>
<table class="r2 hd"><tr>
    <td class="a0">QTY</td><td class="b2">MRP</td><td class="b3">${
		showRack ? "Rack" : ""
	}</td><td class="b4"></td>
</tr></table>
...
function isDraftInvoice(invoiceDoc) {
	return Boolean(
		invoiceDoc.is_draft ||
			...
			(invoiceDoc.name &&
				(invoiceDoc.name.startsWith("DRAFT") || invoiceDoc.name === "DRAFT"))
	);
}
 ...
 */
export async function printInvoiceWithProfileFormat(
	invoiceDoc,
	printFormat = null,
	letterhead = null
) {
