...
/**
 * Docstatus of the given invoices, keyed by name.
 *
 * Names the server does not know are left out of the result. Used to reconcile
 * cached drafts that are bound to a Sales Invoice - see pruneBoundDrafts in
 * stores/posDrafts.js.
 *
 * @param {string[]} invoiceNames
 * @returns {Promise<Object<string, number>>}
 */
export async function getServerDraftStates(invoiceNames) {
	if (!invoiceNames?.length) return {}

	const states = await call("pos_next.api.invoices.get_pos_draft_states", {
