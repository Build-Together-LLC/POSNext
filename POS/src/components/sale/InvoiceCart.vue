			...
			<div v-else class="flex flex-col gap-0.5 sm:gap-1">
				...
				<div
					...
					:class="cartSearchQuery && itemMatchesSearch(item)
						...
						: requireCartItemReview && isItemReviewed(item)
							? 'border-green-300 bg-green-50/40'
							: 'border-gray-200 bg-white'"
				>
					<div class="flex gap-1.5 sm:gap-2">
						<!-- Review Checkbox (POS Setting "Require Cart Item Review"): every line must be ticked before checkout -->
						<div v-if="requireCartItemReview" class="flex-shrink-0 flex items-center" @click.stop>
							<button
								type="button"
								role="checkbox"
		...
		<div class="p-1.5 sm:p-2 bg-white border-t border-gray-200">
			...
			<div class="flex gap-1.5">
				...
				<button
					...
					:class="[
						'flex-1 py-2.5 px-2 rounded-lg font-semibold text-xs transition-all touch-manipulation flex items-center justify-center',
						isSavingDraft
							? 'bg-orange-100 text-orange-400 cursor-not-allowed opacity-75'
							: 'text-orange-700 bg-orange-50 hover:bg-orange-100 active:bg-orange-200 active:scale-[0.98]'
					]"
					:aria-label="isSavingDraft ? __('Saving draft...') : __('Hold order as draft')"
				>
					<svg
						v-if="isSavingDraft"
						class="animate-spin w-4 h-4 me-1.5 text-orange-600"
					...
					<span>{{ isSavingDraft ? __('Saving...') : __('Hold', null, 'order') }}</span>
				</button>
			</div>
		</div>
...
/**
 * POS Setting "Require Cart Item Review". When off, no checkbox or progress
 * line is rendered and checkout only needs a non-empty cart.
 */
...
/** Cart lines not yet ticked — while any exist, checkout stays blocked. */
...
/** Every cart line is ticked. */
const allReviewed = computed(
	() => props.items.length > 0 && unreviewedItems.value.length === 0,
)

/** Tick every line at once, or clear every tick when all are already ticked. */
function toggleAllReviewed() {
	reviewedItems.value = allReviewed.value
		? new Set()
		: new Set(props.items.map(reviewKey))
}
...
function handleCheckout() {
	if (props.isSavingDraft || props.items.length === 0) return
