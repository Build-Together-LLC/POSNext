...
def _get_pos_draft_doc(invoice_name, ptype="read"):
    """Load a held draft after checking it exists, is unsubmitted and is readable."""
    if not invoice_name:
    ...
    if not frappe.has_permission("Sales Invoice", ptype, invoice_name):
        frappe.throw(
            _("You don't have permission to access invoice {0}").format(invoice_name)
        )
...
@frappe.whitelist()
def get_pos_drafts(pos_profile=None, pos_opening_shift=None, limit=100, owner=None):
    ...
    in the browser (see sortByCreatedDesc in stores/posDrafts.js) - so `limit`
    truncates the same tail the cashier would have seen last.
    ...
    parked ticket can be resumed at any till. Pass `owner` to narrow the list to
    a single cashier (delete_all_pos_drafts does this to protect other people's
    drafts).
    """
    fields = [
        "name",
    ...
    # One query for every line of every listed draft (the parents were already
    # permission-checked by get_list above).
    items_by_draft = {}
